...
if [[ ! -f "$ENV_FILE" ]]; then
  echo "Missing $ENV_FILE — run make bootstrap-arbitrum first" >&2
  exit 1
fi
...
# Hook ctor 3rd arg is unused but must match deploy (ANTI_BOT_SELLER or admin).
HOOK_CTOR_THIRD="${ANTI_BOT_OPERATOR:-}"
# openSell.operator == admin in our test env; bootstrap used ANTI_BOT_SELLER env or admin.
if [[ -z "$HOOK_CTOR_THIRD" ]]; then
  # Prefer explicit operator if we stored admin as operator (standard test setup).
  HOOK_CTOR_THIRD="$ADMIN_ADDRESS"
fi
...
# Pxt(admin, donation, marketing, sellUnlock)
ARGS=$(cast abi-encode "constructor(address,address,address,uint256)" \
  "$ADMIN_ADDRESS" "$DONATION_WALLET" "$MARKETING_WALLET" "$SELL_UNLOCK_TIMESTAMP")
...
# PhoenixV4ReturnDeltaHook(manager, pxt, unusedAntiBotOp, admin)
ARGS=$(cast abi-encode "constructor(address,address,address,address)" \
  "$POOL_MANAGER" "$PXT_ADDRESS" "$HOOK_CTOR_THIRD" "$ADMIN_ADDRESS")
...
# PhoenixFeeCollector(poolManager, pxt, donation, marketing, admin)
ARGS=$(cast abi-encode "constructor(address,address,address,address,address)" \
  "$POOL_MANAGER" "$PXT_ADDRESS" "$DONATION_WALLET" "$MARKETING_WALLET" "$ADMIN_ADDRESS")
